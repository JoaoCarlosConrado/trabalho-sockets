...
class activateActuator:
    ...
    def execute(message_json):
        if message_json["actuator_type"] == "heater":
            cf.writeFile("heater", "ACTIVE")
        elif message_json["actuator_type"] == "cooler":
            cf.writeFile("cooler", "ACTIVE")
        elif message_json["actuator_type"] == "irrigationsystem":
            cf.writeFile("irrigationsystem", "ACTIVE")
        elif message_json["actuator_type"] == "co2injector":
            cf.writeFile("co2injector", "ACTIVE")
        response = ActuatorResponseConnection(message_json["key"], "OK", "Actuator activated", message_json["actuator_type"], message_json["actuator_id"])
        return response